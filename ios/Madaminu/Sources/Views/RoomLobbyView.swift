    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: Spacing.md) {
                HStack {
                    Button { controller.leaveRoom() } label: {
                ...
                VStack(spacing: Spacing.xs) {
                    ...
                    Button {
                        UIPasteboard.general.string = controller.roomCode
                        copied = true
                        DispatchQueue.main.asyncAfter(deadline: .now() + 2) { copied = false }
                    } label: {
                        HStack(spacing: Spacing.xs) {
                            Text(controller.roomCode).font(.mdLargeTitle).foregroundStyle(Color.mdPrimary).tracking(8)
                            Image(systemName: copied ? "checkmark" : "doc.on.doc")
                                .font(.mdCaption).foregroundStyle(copied ? Color.mdSuccess : Color.mdTextMuted)
                        }
                    }

                    Text(copied ? "コピーしました" : "\(controller.players.count)人が参加中")
                        .font(.mdCaption).foregroundStyle(copied ? Color.mdSuccess : Color.mdTextMuted)
                }
                ...
                if !controller.hasCreatedCharacter {
                    MDButton("キャラクターを作成") { controller.showCharacterCreation() }
                } else if controller.isHost {
                    MDButton("ゲーム開始", isLoading: controller.isLoading) {
                        Task { @MainActor in await controller.startGame() }
                    }
                    let need = max(0, 4 - controller.players.filter { $0.characterName != nil }.count)
                    if need > 0 {
